...
# videoMerge.py  videoSimpleMerge.py
# 视频合并工具（并行处理和单个处理）  

该 Python 脚本用于将指定文件夹中的多个视频文件合并为一个视频文件，并通过并行处理加速合并过程。该脚本适用于大量视频文件的高效合并。

## 功能特点

- **自定义分辨率和帧率**：用户可以指定合成视频的分辨率和帧率。
- **并行处理**：将视频文件分组并行处理，加速合并过程。
- **保持输入视频顺序**：视频会按照在输入文件夹中的顺序依次合并。

## 依赖项

- **Python 3.x**
- **MoviePy**：一个 Python 视频编辑库。
  - 通过 pip 安装：
    ```bash
    pip install moviepy
    ```
- **FFmpeg**：开源多媒体处理工具。请确保 FFmpeg 已安装并在系统的 PATH 中可用。

## 使用方法

1. 克隆本项目或下载该脚本文件。
2. 根据需要调整输入、输出路径、目标分辨率和帧率。

#  videoSimpleMerge.py
# 视频合并工具（单个处理）  

该 Python 脚本用于将指定文件夹中的单进程串行处理，逐个视频文件合并，适用于小批量视频合并，处理速度较慢-----较少视频建议使用这个
